...
test("api renders data to the DOM", async () => {
...
test("does clicking on Season 1 work", async () => {
  mockFetchShow.mockResolvedValueOnce(mockData);
  const { getByText, getAllByText } = render(<App />);
...
test("does season 4 display properly when clicked on", async () => {
  mockFetchShow.mockResolvedValueOnce(mockData);
  const { getByText, getAllByText } = render(<App />);
  ...
  expect(getAllByText(/minutes/i)).toHaveLength(1);
});
